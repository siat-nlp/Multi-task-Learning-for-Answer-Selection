...
tf.flags.DEFINE_integer("batch_size", 125, "batch size of each batch")
tf.flags.DEFINE_integer("epoches", 300, "epoches")
tf.flags.DEFINE_integer("rnn_size", 1000, "embedding size")
...
tf.flags.DEFINE_integer("run", 0, "matrix size")

tf.flags.DEFINE_float("margin", 0.2, "the proportion of dropout")
# Misc Parameters
tf.flags.DEFINE_boolean("allow_soft_placement", True, "Allow device soft device placement")
tf.flags.DEFINE_boolean("log_device_placement", False, "Log placement of ops on devices")
tf.flags.DEFINE_float("gpu_options", 0.35, "use memory rate")
...
for_test_ori_quests, for_test_cand_quests, for_labels, for_results , for_test_cat_ids = load_test_data(FLAGS.train_LONG, word2idx, FLAGS.num_unroll_steps)#----------------------------------- load data end ----------------------
...
with tf.Graph().as_default():
    with tf.device("/gpu:0"):
        ...
        with tf.Session(config=session_conf).as_default() as sess:
            ...
            logger.info("max acc for TRAIN: {},  MAP: {}, MRR: {}".format(acc_train_max, train_map, train_mrr))            #---------------------------------- end train -----------------------------------
