...
tf.flags.DEFINE_float("lr", 0.3, "the proportion of dropout")
...
tf.flags.DEFINE_float("margin", 0.1, "the proportion of dropout")

# Misc Parameters
tf.flags.DEFINE_boolean("allow_soft_placement", True, "Allow device soft device placement")
tf.flags.DEFINE_boolean("log_device_placement", False, "Log placement of ops on devices")
tf.flags.DEFINE_float("gpu_options", 0.85, "use memory rate")
...
ori_quests, cand_quests,neg_quests, q_cats,a_cats = load_train_data(FLAGS.train_file, word2idx, FLAGS.num_unroll_steps)

test_ori_quests, test_cand_quests, labels, results , test_q_cats,test_a_cats = load_test_data(FLAGS.test_file, word2idx, FLAGS.num_unroll_steps)


for_test_ori_quests, for_test_cand_quests, for_labels, for_results , for_test_q_cats,for_test_a_cats = load_test_data(FLAGS.train_LONG, word2idx, FLAGS.num_unroll_steps)
...
#----------------------------------- execute train model ---------------------------------
def run_step(sess, ori_batch, cand_batch, neg_batch,q_cats_batch,a_cats_batch, lstm, dropout, is_optimizer=True,print_log=True):
    start_time = time.time()
    q_cats_batch_onehot = onehot_encoder(q_cats_batch)
    a_cats_batch_onehot = onehot_encoder(a_cats_batch)
    ...
    if is_optimizer:
        feed_dict = {
            ...
            lstm.q_cats: q_cats_batch_onehot,
            lstm.a_cats: a_cats_batch_onehot
        }
...
#---------------------------------- execute valid model ------------------------------------------
def valid_model(sess, lstm, valid_ori_quests, valid_cand_quests, labels, results,test_q_cats,test_a_cats):
    total_loss, idx = 0, 0
    total_ori_cand = []
    #total_right, total_wrong, step = 0, 0, 0, 0
    for ori_valid, cand_valid, neg_valid,q_cats_test,a_cats_test in batch_iter(valid_ori_quests, valid_cand_quests, test_q_cats,test_a_cats,FLAGS.batch_size, 1,isvalid=True):
        loss, ori_cand = run_step(sess, ori_valid, cand_valid, cand_valid,q_cats_test,a_cats_test, lstm, FLAGS.dropout, False,False)
        total_loss += loss
...
with tf.Graph().as_default():
    with tf.device("/gpu:0"):
        ...
        with tf.Session(config=session_conf).as_default() as sess:
            ...
            for ori_train, cand_train, neg_train, train_q_cats,train_a_cats in batch_iter(ori_quests, cand_quests, q_cats,a_cats, FLAGS.batch_size, FLAGS.epoches, neg_quests=neg_quests):
                run_step(sess, ori_train, cand_train, neg_train, train_q_cats,train_a_cats, lstm, FLAGS.dropout)
            ...
            logger.info("start to evaluation model")
            # valid_model(sess, lstm, valid_ori_quests, valid_cand_quests)
            acc, MAP, MRR = valid_model(sess, lstm, test_ori_quests, test_cand_quests, labels, results, test_q_cats,test_a_cats)
            if acc_test_max < acc:
                acc_test_max = acc
                test_map = MAP
                test_mrr = MRR
            ...
            acc, MAP, MRR = valid_model(sess, lstm, for_test_ori_quests, for_test_cand_quests, for_labels, for_results,
                                        for_test_q_cats, for_test_a_cats)
            if acc_train_max < acc:
                acc_train_max = acc
                train_map = MAP
                train_mrr = MRR
