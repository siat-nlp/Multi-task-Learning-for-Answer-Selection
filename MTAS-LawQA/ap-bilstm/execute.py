...
tf.flags.DEFINE_float("lr", 0.001, "the proportion of dropout")
tf.flags.DEFINE_integer("batch_size", 500, "batch size of each batch")
...
tf.flags.DEFINE_integer("quest_len", 100, "embedding size")
...
tf.flags.DEFINE_float("margin", 0.1, "margin")
...
with tf.Graph().as_default():
    with tf.device("/gpu:0"):
        ...
        with tf.Session(config=session_conf).as_default() as sess:
            lstm = LSTM(FLAGS.batch_size, FLAGS.quest_len, FLAGS.answer_len, embedding, FLAGS.embedding_size, FLAGS.rnn_size, FLAGS.num_rnn_layers, FLAGS.max_grad_norm,ratio,m=FLAGS.margin)
