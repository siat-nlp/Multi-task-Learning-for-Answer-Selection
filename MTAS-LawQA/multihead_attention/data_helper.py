...
def load_train_data(filename, word2idx, sequence_len):
    ...
    load train data
    """
    ori_quests, cand_quests, neg_quests,q_cats,a_cats = [], [], [],[],[]
    with codecs.open(filename, mode="r", encoding="utf-8") as rf:
        try:
            for line in rf.readlines():
    ...
    return ori_quests, cand_quests, neg_quests , q_cats,a_cats
...
def load_test_data(filename, word2idx, sequence_len):
    ...
    load test data
    """
    ori_quests, cand_quests, labels, results,q_cats,a_cats = [], [], [],[],[],[]
    with codecs.open(filename, mode="r", encoding="utf-8") as rf:
        try:
            for line in rf.readlines():
                arr = line.strip().split(" ")
    ...
    return ori_quests, cand_quests, labels, results , q_cats,a_cats

def batch_iter(ori_quests, cand_quests,q_cats,a_cats, batch_size, epoches, neg_quests=[],isvalid=False):
    """
    iterate the data
    """
    ...
    q_cats = np.array(q_cats)
    a_cats = np.array(a_cats)
    ...
    for epoch in range(epoches):
        for batch in range(batch_num):
            ...
            yield (ori_quests[start_idx : end_idx], cand_quests[start_idx : end_idx], neg_quests_one_batch, q_cats[start_idx : end_idx], a_cats[start_idx : end_idx])
